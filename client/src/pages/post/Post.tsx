         ...
         {post && (
            <div className="flex flex-col items-start w-full gap-6 px-5 py-5 2xl:flex-row xl:flex-row 2xl:px-0 md:px-0">
               <div className="bg-lightV1 flex-1 rounded-sm p-3 flex flex-col gap-2.5 w-full">
                  <span>{post.text}</span>
                  {post.image && (
                     <img className="w-full rounded-sm" src={process.env.REACT_APP_API_URL + post.image} alt={post.owner.name} />
                  )}
                  ...
                  <div className="flex items-center justify-around pt-2 text-sm border-t 2xl:text-base sm:text-base border-t-lightV4 text-mainDarkV1">
                     <button onClick={() => likehandle(post._id)} className="flex items-center gap-2 rounded-sm bg-lightV3 group">
                        <span className="hidden pl-2 2xl:inline-block sm:inline-block">{post.likes.count}</span>
                     ...
                     <button className="flex items-center gap-2 rounded-sm bg-lightV3 group">
                        <span className="hidden pl-2 2xl:inline-block sm:inline-block">{post.comments.count}</span>
               ...
               <div className="bg-lightV1 rounded-sm flex flex-col justify-between flex-[0.5] p-3 gap-3 w-full">
                  <div className="">
                     <NavLink to={`/user/${post.owner._id}`} className="flex gap-1.5 items-center">
                        <Avatar name={post.owner.name} src={post.owner.avatar} size={40} variant="beam" />
                        <div className="flex flex-col">
                           <p className="leading-4">{post.owner.name}</p>
                           <span className="text-sm text-mainDarkV1/50">{formatDate(post.createdAt)}</span>
                        </div>
                     </NavLink>
                  </div>
                  <form
                  ...
                  >
                     <textarea
                        ...
                        name="text"
                     ></textarea>
                     <button className="w-full py-1 ml-auto rounded-sm px-7 bg-mainDarkV2 2xl:w-auto sm:w-auto text-lightV1" type="submit">
                        Send
                     </button>
                  </form>
                  <ul className="flex flex-col flex-1 gap-2 rounded-sm">
                     {post.comments.comment && (
                        <>
                           {post.comments.comment.comments.map((comment: any, index: number) => (
                              <li className="flex gap-2 p-2 rounded-sm " key={index}>
                                 <Avatar
                                    name={`${FindUser(allUser, comment.user)?.name}`}
                                 ...
                                 <div className="w-full text-sm text-mainDarkV1">
                                    <div className="flex justify-between">
                                       <span className="text-base font-medium">{FindUser(allUser, comment.user)?.name}</span>
                                       <span className="font-semibold text-mainDarkV1/50">{formatDateMin(comment.createdAt)}</span>
                                    </div>
                                    <span className="flex-1 py-1 rounded-sm text-mainDarkV1/80">{comment.text}</span>
                                 </div>
                              </li>
                           ))}
                        </>
